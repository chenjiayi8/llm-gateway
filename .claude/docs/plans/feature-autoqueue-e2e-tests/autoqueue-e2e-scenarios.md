...
| queue-depth overflow run | 50 requests / concurrency 50 (`overflow`) | Mix of `200` and bounded failure responses (`503` preferred; throttling `429` acceptable) once queue ceiling/pressure is hit | Endpoint responds `200`; during pressure, `queued` and/or `local_waiters` rises; after load stops, queue drains to 0 | Spend logs show accepted requests and bounded failures with timestamps around overload window |
| timeout run with intentionally slow upstream | 20 requests / concurrency 20 (`timeout`, `expect_timeout=true`) | Mix of `200` and timeout-class failures with at least one `504` when environment is intentionally slow | Endpoint responds `200`; `active` may stay elevated longer; queue should still drain after timeout horizon | Spend logs capture timeout/error status metadata plus latency values demonstrating slow-upstream behavior |
