...
- PostgreSQL schema authority lives in `platform/schema/litellm`
...
- **Keep schema files in sync.** Land PostgreSQL schema changes in `platform/schema/litellm`, then sync the generated `schema.prisma` and migration assets to the LiteLLM service copies (`schema.prisma`, `litellm/proxy/`, `litellm-proxy-extras/`, `litellm-js/spend-logs/` for SpendLogs).
